...
export default class App extends Component {
  ...
  validateForm = (name,amount) => {
    if (!name || !amount) {
      ...
      window.alert('Check Transaction Name: Text only !!!');
      return false;
    } else if (+amount === 0) {
      window.alert('Check Amount: Do not fill 0 !!!');
      return false;
    }
    else if(!Number.isInteger(Number(amount))){
      window.alert('Check Amount: Cannot be Float !!!')
      return false
    }
  ...
  clearTransactions = () => {
    let ans = window.confirm("** OK to Clear your list **")
    if (ans) {
